...
# def pplGroup(x,i):
#     if i-x==0:
#         sheet_obj.cell(row=x,column=17).value=sheet_obj.cell(row=x,column=16).value
#     else:
#         for a in range(x,i+1):
#             pplArr=[]
#             pplArr.append(sheet_obj.cell(row=a,column=16).value)
#             for b in range(x,i+1):
#                 if sheet_obj.cell(row=b,column=16).value not in pplArr:
#                     pplArr.append(str(sheet_obj.cell(row=b,column=16).value))
#             pplStr=cleanPpl(pplArr)
#             sheet_obj.cell(row=a,column=17).value=pplStr
#     return
