...
path = "E:\\Pham Thanh Quyet - 23.12.2022\\DSKH 22.12.23\\VRS VRH\\23.03.06 Riverside+ Harmony Full - Tổng hợp khách hàng và căn V21 - processed.XLSX"
...
#     sheet_obj.cell(row = x, column = 14).value = houseCount.count(';') + 1
...
for x in range(2,sheet_obj.max_row+1):
    ...
    for i in range(7,13):
        if sheet_obj.cell(row = x, column = i).value > 0:
            count += 1
    sheet_obj.cell(row = x, column = 15).value = count
