...
path = "E:\\Pham Thanh Quyet - 23.12.2022\\DSKH 22.12.23\\VRS VRH\\23.03.16 Riverside+ Harmony Full - Tổng hợp khách hàng và căn V22 - processed.XLSX"
...
for x in range(2,sheet_obj.max_row+1):
    for i in range(x+1,x+5000):
        if sheet_obj.cell(row=i,column=5).value != sheet_obj.cell(row=x,column=5).value:
            ...
            if sheet_obj.cell(row=i,column=4).value != sheet_obj.cell(row=x,column=4).value and sheet_obj.cell(row=i,column=18).value != sheet_obj.cell(row=x,column=18).value:
                break
            else:
