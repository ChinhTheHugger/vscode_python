...
# these methods don't require both linestrings to have similar number of points


# Hausdorff - return value in degree
...
# Frechet - return value in degree

def euclidean_distance(p1, p2):
    return np.sqrt((p2[0] - p1[0])**2 + (p2[1] - p1[1])**2)

def frechet_distance(line1, line2):
    P = [(lon,lat) for lon,lat in line1.coords] 
    Q = [(lon,lat) for lon,lat in line2.coords] 
    ...
    for i in range(1, n):
        for j in range(1, m):
            dp[i][j] = max(min(dp[i-1][j], dp[i-1][j-1], dp[i][j-1]), euclidean_distance(P[i], Q[j]))
...
# - euclidean: degree
# - manhattan_distanec: degree
# - chebyshev_distance: degree
# - minkowski: degree
# - cosine_distance: ratio
# - haversine_distance: m, km,...

def dtw_distance(line1, line2):
    linestring1 = [(lon,lat) for lon,lat in line1.coords] 
    linestring2 = [(lon,lat) for lon,lat in line2.coords]
    
    linestring1_np = np.array(linestring1)
    linestring2_np = np.array(linestring2)

    distance, path = fastdtw(linestring1_np, linestring2_np, dist=euclidean)
    
    return distance


# Overlay Analysis - return value in multipoint, multilinestring, etc
...
print(hausdorff_distance(linestring1,linestring2),end="\n")
print(average_hausdorff_distance(linestring1,linestring2),end="\n")

print(frechet_distance(linestring1,linestring2),end="\n")

print(dtw_distance(linestring1,linestring2),end="\n")

print(overlay_analysis(linestring1,linestring2)['intersection'],end="\n")
print(overlay_analysis(linestring1,linestring2)['union'],end="\n")
print(overlay_analysis(linestring1,linestring2)['difference'],end="\n")
...
# NOTES:
# - Try getting max, min, mean and median values of Hausdorff distance
# - Then we can use statistical methods to analyze the data
# - Consider the above linestrings
# - We can already tell they are nearly identical, except for one small section
# - In theory, we can expect the max Hausdorff and min Hausdorff to vary greatly
# - And the mean and median Hausdorff are most likely smaller than the half value of max Hausdorff
# - We can refine this further and output a set of rules to decide whether or not two given linestrings are similar
# - If we were to graph the Hausdorff values of the above linestrings, we can probably expect its peak to lean to the right, and a long tail on the left
# - We can expect any given pair of linestrings that follows this form to be reasonably similar for the most part, and shorter peak means the noticable differences are less... noticable
# - The ideal graph form should be more akin to a horizontal line, which means all Hausdorff distances are both roughly the same and very small